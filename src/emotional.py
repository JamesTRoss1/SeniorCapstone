...
def emotion_faces(faces):
    ...
    :param list faces: a numpy array in BGR format or a base64 encoded image
    ...
    preds = model.analyze(faces, actions = ['emotion'], detector_backend="mtcnn")
    emotions = {}
    weights = {'sad': 1, 'angry': 1, 'surprise': 1, 'fear': 1, 'happy': 1, 'disgust': 1, 'neutral': 1}

    print(str(preds))
    for pred in preds:
        #reliability of the model, don't process bad data  
        if pred["face_confidence"] < 0.9:
            pass 
        else:
